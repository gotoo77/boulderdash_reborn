...
void pushDirection(std::vector<Direction>& stack, Direction dir) {
    const auto existing = std::find(stack.begin(), stack.end(), dir);
    if (existing != stack.end()) {
        stack.erase(existing);
    }
    stack.push_back(dir);
}

void releaseDirection(std::vector<Direction>& stack, Direction dir) {
    const auto existing = std::find(stack.begin(), stack.end(), dir);
    if (existing != stack.end()) {
        stack.erase(existing);
    }
...
std::optional<Direction> currentHeldDirection(const std::vector<Direction>& stack) {
    if (stack.empty()) {
        return std::nullopt;
    }
    return stack.back();
...
int main() {
    ...
    bool running = true;
    std::vector<Direction> heldDirections;
    Uint32 lastTick = SDL_GetTicks();
    Uint32 lastAutoMove = SDL_GetTicks();
    std::optional<Direction> lastAutoDirection;
    ...
    while (running) {
            ...
            if (screen == ScreenState::Menu) {
                ...
                if (event.type == SDL_KEYDOWN && event.key.repeat == 0) {
                    if (auto dir = directionFromKey(event.key.keysym.sym)) {
                        ...
                        lastAutoMove = SDL_GetTicks();
                    }
                } else if (event.type == SDL_KEYUP) {
                    if (auto dir = directionFromKey(event.key.keysym.sym)) {
                        releaseDirection(heldDirections, *dir);
                        if (lastAutoDirection && *lastAutoDirection == *dir) {
                            lastAutoDirection.reset();
                        }
                    }
                }
        ...
        if (screen == ScreenState::Playing) {
            if (auto held = currentHeldDirection(heldDirections)) {
                if (!lastAutoDirection || *lastAutoDirection != *held) {
                    lastAutoDirection = held;
                    lastAutoMove = now;
                } else if (now - lastAutoMove >= static_cast<Uint32>(config.moveRepeatMs)) {
                    game.queueMove(*held);
                    lastAutoMove = now;
                }
            } else {
            ...
            if (now - lastTick >= static_cast<Uint32>(config.tickMs)) {
                const Uint32 delta = now - lastTick;
                game.update(delta);
                lastTick = now;
            }
